  ...
  async function addRepository(repositoryName: string) {
    try {
      ...
      if (repoAlreadyExists) {
        return Alert.alert('Erro ao cadastrar repositório', 'Esse repositório já está cadastrado.');
      }

      const response = await api.get<RepositoryProps>(`orgs/${repositoryName}/repos`);
      console.log(response.data);

      const { data: issues } = await api.get<IssueProps[]>(`repos/facebook/react/issues`);
      setRepositories([
        ...repositories,
        {
          ...response.data,
